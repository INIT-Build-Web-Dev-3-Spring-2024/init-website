...
export const ProgramCard: React.FC<ProgramProps> = ({ program }) => {
	const { title, image, color, url, open } = program;

	return (
		<div
			key={title}
			className="text-gray-400 h-auto transition delay-500 duration-1000 relative mx-auto mt-4 w-full"
		>
			<div className="flex flex-grow h-full flex-col overflow-hidden rounded-t-xl bg-zinc-800  shadow">
				<button onClick={() => {}}>
					<div className="relative h-56 w-full overflow-hidden ">
						<Image
							className="max-[2560px]:object-top min-[320px]:object-left sm:object-left object-cover md:object-left  2xl:object-top shadow-[0_10px_10px_black]"
							alt={title}
							src={image}
							fill
							sizes="100vw"
				...
				<div className=" flex flex-grow flex-col p-5">
					...
					<p
						className="text-md mb-3 h-[100px] flex-grow overflow-y-auto font-semibold"
					>
						{program.description}
					</p>
					<div className="mb-2 self-start">
						{open ? (
							<a
								...
								className={`inline-flex w-full items-center justify-center rounded-lg px-3 py-2 text-center text-sm font-medium text-white hover:bg-opacity-90 focus:outline-none focus:ring-4 focus:ring-opacity-50`} style={{ backgroundColor: color }}
							> 
								Apply Now
								{/* SVG Icon */}
							</a>
