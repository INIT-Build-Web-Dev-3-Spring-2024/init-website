...
export default async function fetchEvents(searchQuery: eventSearchProperties, weekly = false) {
  try {
        ...
        (page: PageObjectResponse | PartialPageObjectResponse | PartialDatabaseObjectResponse | DatabaseObjectResponse) => {
          if ('properties' in page) {
            ...
            if (dateProperty?.type === 'date' && dateProperty.date) {
              ...
              const dateObject = timeProvided ? new Date(dateProperty.date.start) : new Date(`${dateProperty.date.start}T12:00:00.000Z`) // if no time is provided, create date as if time is 12 PM UTC so that the correct day is rendered
              ...
              if (dateProperty.date.end) {
                ...
                const dateEndObject = timeEndProvided ? new Date(dateProperty.date.end) : new Date(`${dateProperty.date.end}T12:00:00.000Z`)
