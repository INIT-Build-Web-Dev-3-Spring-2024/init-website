...
export default function Navbar() {
  ...
  return (
    ...
    >
        ...
        <ul className="flex mx-auto justify-center items-center gap-2 w-fit border border-secondary-gray rounded-3xl p-1 bg-page shadow-2xl shadow-black">
          {navLinks.map(({ label, href }) => (
            <li key={label} className="relative">
              {pathName == href && (
                <motion.span layoutId="highlight" className={'absolute bottom-full w-full'}>
      ...
      >
        <GradientBorder className="px-3 py-1 rounded-3xl cursor-pointer" animatedOnHover>
          <Link href="https://airtable.com/appkfpQOssQZfmORj/shrNlrSaT073i6fog" className="p-0">
            Get INIT
          </Link>
        </GradientBorder>
      </div>
